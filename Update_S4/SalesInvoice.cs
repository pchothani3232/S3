    ...
    public partial class SalesInvoice : Form
    {
        ...
        private bool SaveData()
        {
            foreach (DataGridViewRow row in SalesInvoiceDetailDGV.Rows)
            {
                ...
                var netAmountCell = row.Cells["ProductNetAmount"];
                if (netAmountCell.Value == null || Convert.ToDecimal(netAmountCell.Value) == 0)
                {
                    MessageBox.Show("Cannot save data. One or more rows have a Product Net Amount of zero.", "Save Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            if (SalesInvoiceDetailDGV.SelectedRows.Count > 0)
            {
                var selectedRow = SalesInvoiceDetailDGV.SelectedRows[0];
                if (!selectedRow.IsNewRow)
                {
                    var result = MessageBox.Show("Are you sure you want to delete this row?", "Confirm Deletion", MessageBoxButtons.YesNo);
                    if (result == DialogResult.Yes)
                    {
                        SalesInvoiceDetailDGV.Rows.RemoveAt(selectedRow.Index);
                    }
                }
            }
        }
